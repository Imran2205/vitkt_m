...
@register_model
def vittracking_base_patch16(pretrained=False, pretrain_type='default',
    ...
    cfg_type = 'vittracking_base_patch16_224_' + pretrain_type
    model.default_cfg = default_cfgs[cfg_type]
    if pretrained:
        side_num_patches_search = search_size // patch_size
        side_num_patches_template = template_size // patch_size
        load_pretrained(model, side_num_patches_search,  side_num_patches_template,
                        pretrain_type, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3))
    return model

def vittracking_large_patch16(pretrained=False, pretrain_type='default',
                              search_size=384, template_size=192, **kwargs):
    patch_size = 16
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vittracking_large_patch16_224_' + pretrain_type
    model.default_cfg = default_cfgs[cfg_type]
    if pretrained:
        side_num_patches_search = search_size // patch_size
        side_num_patches_template = template_size // patch_size
...
def vittracking_huge_patch14(pretrained=False, pretrain_type='default',
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    cfg_type = 'vittracking_huge_patch14_224_' + pretrain_type
    model.default_cfg = default_cfgs[cfg_type]
    if pretrained:
        side_num_patches_search = search_size // patch_size
        side_num_patches_template = template_size // patch_size
...
def load_pretrained(model, side_num_patches_search, side_num_patches_template, pretrain_type='default', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    if cfg is None:
    ...
    if pretrain_type == "mae":
        ...
        # strict = False
    # if state_dict['pos_embed'].shape[1] != side_num_patches_search ** 2 + side_num_patches_template ** 2 + 1:
